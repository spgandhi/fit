...
function NewWorkout(props:Props) {
  ...
  if(exercisesData) {
    exercisesData.exercises.forEach((element: any) => {
  ...
  return (
    <>
      {exercisesData &&
      <div>
        <div className="sticky top-0 py-4 z-10 bg-white flex flex-col gap-y-4 px-4">
          <div>
          ...
          {selectedExercisesList && 
            <div>
              {selectedExercisesList.map((item) => (
                <IonChip className="rounded-md my-1" onClick={() => onExerciseSelect(item.id)}>
                  {item.name} <IonIcon className="text-sm" icon={close} />
                </IonChip>
              ))}
            </div>
          }
