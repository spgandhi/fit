...
import { Exercise, ExerciseRound, User, Workout } from '../../data/types';
...
function WorkoutSession(props: Props) {
  ...
  const updateRoundDataLocally = (exerciseId: string, roundData: any) => {
    ...
    let exerciseIndex = newWorkoutData?.exercises?.findIndex(exercise => exercise.id === exerciseId);
    if(!exerciseIndex || !newWorkoutData) return;
    
    if(!newWorkoutData.exercises[exerciseIndex].rounds) {
      newWorkoutData.exercises[exerciseIndex].rounds = [];
    }
  ...
  const handleEndWorkout = () => {
    ...
    history.push(`${userPath}/workouts`); 
  }
